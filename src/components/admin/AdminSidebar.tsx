import { LayoutDashboard, Inbox, ShieldCheck, Settings } from 'lucide-react';
...
export function AdminSidebarDesktop({ activeTab, setActiveTab, pendingCount, currentAdmin, t }: any) {
  const sidebarItems = [
    ...
    { id: 'requests', label: t('pendingRequests'), icon: <Inbox size={18} /> },
    { id: 'settings', label: t('accountSettings'), icon: <Settings size={18} /> },
  ];
      ...
      <nav className="p-3 space-y-1">
        {sidebarItems.map(item => (
          <button 
            ...
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${activeTab === item.id ? 'text-white bg-black' : 'opacity-60 hover:opacity-100 text-[var(--ct-text)] bg-transparent'}`}
          >
            {item.icon}
            {item.label}
    ...
    <div className="md:hidden flex gap-2 mb-6 overflow-x-auto pb-2">
      {sidebarItems.map(item => (
        <button 
          key={item.id} 
          onClick={() => setActiveTab(item.id as AdminTab)} 
          className={`flex-shrink-0 px-3 py-2 rounded-lg text-xs font-medium transition-all ${activeTab === item.id ? 'text-white bg-black' : 'opacity-60 text-[var(--ct-text)] bg-[var(--ct-surface)]'}`}
          title={item.label}
        >
          {item.icon}
        </button>
